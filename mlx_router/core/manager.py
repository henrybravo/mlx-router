...
from mlx_router.core.content import decode_base64_to_images
...
class MLXModelManager:
    ...
    def get_health_metrics(self):
        ...
        return {
            ...
            "memory_health": {
                ...
                "can_load_models": [
                    model
                    for model in ModelConfig.get_available_models()
    ...
    def load_model(self, model_name):
        ...
        is_vision = model_config.get("supports_vision", False) or any(
            keyword in model_name.lower() for keyword in ["vl", "vision", "llava", "qwen-vl", "nvl"]
        )

        if is_vision and not _vision_available:
            raise RuntimeError("Vision model support not available. Install mlx-vlm>=0.3.9")
        ...
        with self.model_lock:
            ...
            try:
                ...
                if is_vision:
                    ...
                    self.current_tokenizer = None
                    logger.info("Loaded as vision model")
                else:
                ...
                mem_info = ResourceMonitor.get_memory_info()
                logger.info(
                    f"✅ Loaded {model_name} in {time.time() - start_time:.2f}s (Memory: {mem_info['used_percent']:.1f}%)"
                )
                return True
            except Exception as e:
    ...
    def _format_messages(self, messages, tools=None):
        ...
        if (
            template.get("use_tokenizer_template")
            and self.current_tokenizer
            and hasattr(self.current_tokenizer, "apply_chat_template")
        ):
            try:
    ...
    def _parse_tool_calls(self, text: str, tools: list[dict]) -> dict[str, Any] | None:
    ...
    def _generate_response_impl(
        ...
        max_tokens_val = (
            max_tokens if max_tokens is not None else model_config.get("max_tokens", self.default_max_tokens)
        )
        ...
        if pressure_level != "normal":
            pressure_max_tokens = ResourceMonitor.get_memory_pressure_max_tokens(
                self.current_model_name, pressure_level
            )
            if pressure_max_tokens < max_tokens_val:
                logger.warning(
                    f"Memory pressure '{pressure_level}' detected. Reducing max_tokens from {max_tokens_val} to {pressure_max_tokens}"
                )
        ...
        try:
            ...
            return {"type": "text", "content": self._sanitize_response(raw_response), "finish_reason": "stop"}
    ...
    def _generate_with_mlx(
        self, prompt, max_tokens, temperature, top_p, top_k, min_p, stream=False, stream_chunk_size=None
    ):
        ...
        try:
            ...
            if stream:
                ...
                num_tokens_generated = (
                    len(self.current_tokenizer.encode(response)) if self.current_tokenizer else len(response.split())
                )
                logger.info(
                    f"Generated ~{num_tokens_generated} tokens in {gen_time:.2f}s using {self.current_model_name}"
                )
                return response
        ...
        for img_data in images:
            ...
            if img_data.startswith("data:"):
                try:
                    ...
                    raise RuntimeError(
                        f"PDF processing requires pdf2image. Install with: pip install pdf2image. Error: {e}"
                    ) from e
                except Exception as e:
    ...
    def _generate_with_vlm(self, prompt, max_tokens, temperature, top_p, top_k, min_p, images=None, stream=False):
        ...
        try:
            ...
            image_arg = (
                processed_images[0] if len(processed_images) == 1 else (processed_images if processed_images else None)
            )
            ...
            formatted_prompt = apply_vision_chat_template(
                self.vision_processor, self.vision_config, prompt, num_images=num_images
            )
            ...
            if stream:
                ...
                logger.info(
                    f"Generated vision response in {gen_time:.2f}s using {self.current_model_name} ({response.generation_tokens} tokens, {response.generation_tps:.1f} tokens/s)"
                )
                return result_text
