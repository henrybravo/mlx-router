...
mlx_model = "mlx-community/NVIDIA-Nemotron-3-Nano-30B-A3B-4bit"
...
# Async function to invoke prompts with streaming
async def run_prompts():
    try:
        # Zero-shot example (streaming)
        print("\n=== Zero-Shot Intent Detection (Streaming) ===")
        ...
        async for chunk in kernel.invoke_stream(
            ...
            # Extract content from the first StreamingChatMessageContent in the list
            content = chunk[0].content if chunk and len(chunk) > 0 and chunk[0].content else ""
            print(content, end="", flush=True)
        print()

        # Few-shot example (streaming)
        print("\n=== Few-Shot Intent Detection (Streaming) ===")
        print(f"Input: Draft a proposal for the new project")
        async for chunk in kernel.invoke_stream(
            few_shot_function,
            request="Draft a proposal for the new project",
        ...
        # Persona example (streaming)
        print("\n=== Persona-Based Explanation (Streaming) ===")
        ...
        # Chain-of-thought example (streaming)
